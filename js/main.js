!(function ($) {
  ...
  $("#send_message_btn").on("click", function () {
    ...
    let phone_number = document.getElementById("phoneNbr").value;
    let message = document.getElementById("message").value;

    let regex = /^[^\s()<>@,;:\/]+@\w[\w\.-]+\.[a-z]{2,}$/;
    if (email.match(regex) && phone_number.length >= 8 && firstname.length >= 3 && lastname.length >= 3 && message.length >= 1) {
      $.ajax({
